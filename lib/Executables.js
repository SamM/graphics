    ...
    function runCode(makeMessages){
        try{
            ...
            if(typeof return_value != "undefined"){
                if(makeMessages!==false) makeMessage("Result", _(return_value).toString(1), "resultMessage", "hsl(100, 80%, 45%)");
            }
        }catch(err){
            if(makeMessages!==false) makeErrorMessage(err.name+(typeof err.lineNumber=="number"?"@ "+err.lineNumber+":"+err.columnNumber:""), err.message);
        }
    }
