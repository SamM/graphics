...
/*/
        ...
        getterSetter.__defineGetter__('_', function(){
            return one;
        });
        getterSetter.__defineSetter__('_', function(two){
            one = two;
            getSet(one);
        });
    ...
    if(Array.isArray(value)){
        ...
        for(i = 0; i < value.length; i++){
            ...
            if(typeof two == "string"){
                ...
                two = Variable(two).toString(padding_min, padding_max);
            }
            values.push(two);
        }
        return ["[", values.join(_+","+_), "]"].join(_);
    }else if(typeof value == "function"){
        return Function.prototype.toString.call(value).split(/[\ ]+/).join(' ');
        ...
        for(var key in value){
            ...
            pairs.push([key.replace(/\\"/g, '\\\\"').replace(/"/g, '\\"'), two].join(_+':'+_));
        }
        return ["{",pairs.join(_+","+_),"}"].join(_);
